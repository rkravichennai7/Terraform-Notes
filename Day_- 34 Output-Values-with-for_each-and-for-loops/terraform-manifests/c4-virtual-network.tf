...
## 2. for_each = var.environment

- for_each is a meta-argument in Terraform that allows you to iterate over a map or a set of strings to create multiple instances of a resource/module dynamically.
- var.environment is a Terraform input variable that is expected to be a map or a set of values.
- Terraform will create a separate resource for each key-value pair in var.environment.
